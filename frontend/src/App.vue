...
const data = reactive({ health: null, baseImages: [], marketplaceImages: [], images: [], operations: [], vms: [], servers: [], logs: [], pull: null, message: '', runtime: {}, metrics: { workers: 0, microvms: 0, runningVms: 0, images: 0, operations: 0, timestamp: null, host: {} } })
...
async function pullImage(id) {
  const item = data.marketplaceImages.find(image => image.id === id)
  busy.value = true
  data.pull = { id, name: item?.name || id, phase: 'starting', logs: [`Starting download for ${item?.name || id}`, 'Fetching verified marketplace catalog entry…'] }
  try {
    data.pull.phase = 'downloading'
    data.pull.logs.push('Downloading kernel…', 'Downloading rootfs…', 'Verifying SHA-256 checksums…')
    const result = await PullMarketplaceImage(id)
    data.pull.phase = 'completed'
    data.pull.logs.push(`Stored image directory: ${result.directory || 'runtime image store'}`, 'Checksums verified', 'Image registered in local store')
    data.message = `${item?.name || id} downloaded and verified`
    await refresh()
  } catch (error) {
    data.pull.phase = 'failed'
    data.pull.logs.push(`Download failed: ${String(error).replace(/^Error: /, '')}`)
    data.message = `Image pull failed: ${String(error).replace(/^Error: /, '')}`
...
<template>
  <div class="desktop-shell">
    <aside class="sidebar">
      ...
      <div class="sidebar-bottom"><small>v1.3.0 · BlackLoverTech</small></div>
    </aside>
