...
async function createVM() {
  ...
  busy.value = true
  try { const vm = await CreateVM(operation.artifact.digest, Number(state.vcpus), Number(state.memoryMiB), operation.request.source); state.selectedVM = vm.id; state.tab = 'vms'; data.message = `Created workload from ${operation.request.source}`; await refresh() } catch (error) { data.message = String(error) } finally { busy.value = false }
}
...
<template>
  <div class="desktop-shell">
    <aside class="sidebar">
      ...
      <div class="sidebar-bottom"><small>v1.1.1 · BlackLoverTech</small></div>
    </aside>
