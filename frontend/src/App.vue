...
async function deleteVM(id) { if (!window.confirm('Delete this microVM and its managed runtime files?')) return; try { await DeleteVM(id); state.selectedVM = null; data.message = 'Workload deleted'; await refresh() } catch (error) { data.message = String(error) } }
...
<template>
  <div class="desktop-shell">
    <aside class="sidebar">
      ...
      <div class="sidebar-bottom"><small>v1.4.0 · local control server</small></div>
    </aside>
    ...
    <main class="main-area">
      ...
      <div class="content">
        <template v-if="state.tab === 'overview'">
          <section class="hero compact"><div><p class="eyebrow">FIRECRACKER CONTROL CENTER</p><h2>Run isolated workloads.</h2><p>{{ data.message || 'Firecracker Studio controls the runtime already installed on this host.' }}</p></div><button class="primary small" @click="state.tab = 'convert'">Convert image</button></section>
        ...
        <template v-else-if="state.tab === 'images'"><section class="toolbar"><div><p class="eyebrow">IMAGE STORE</p><h2>Images</h2><p class="section-note">Use familiar OCI names. Studio keeps the source name and adds the Firecracker artifacts behind it.</p></div><button class="primary small" @click="state.tab = 'convert'">Pull image</button></section><section class="grid two"><article class="panel"><div class="panel-head"><div><p class="eyebrow">LOCAL STORE</p><h2>Downloaded images</h2></div><span class="chip">{{ data.images.length }} local</span></div><div v-for="image in data.images" :key="image.digest" class="operation"><div class="operation-main"><strong>{{ image.reference }}</strong><small class="mono">{{ image.sourceType }} · {{ image.digest?.slice(0, 16) }}</small></div><span class="state succeeded">ready</span></div><div v-if="!data.images.length" class="empty">No local images yet. Pull an OCI image to create a reusable Firecracker image.</div></article><article class="panel"><div class="panel-head"><div><p class="eyebrow">STARTER IMAGES</p><h2>Built-in defaults</h2></div><span class="chip">installed runtime</span></div><p class="section-note">Firecracker Studio does not install Firecracker. Place compatible kernel and rootfs assets in the runtime directory, then use them here.</p><div v-for="image in data.baseImages" :key="image.id" class="operation starter-image-row"><div class="operation-main starter-image-main"><strong class="starter-image-name">{{ image.name || image.id || 'Unnamed starter image' }}</strong><small class="starter-image-meta">{{ image.distribution || 'Firecracker' }} {{ image.version || '' }} · {{ image.architecture || 'x86_64' }}</small><small class="starter-image-id mono">{{ image.id }}</small></div><span class="state" :class="image.status">{{ image.kernelUrl && image.rootfsUrl ? 'ready' : 'info' }}</span></div><div v-if="!data.baseImages.length" class="empty">No runtime assets are available from this server.</div></article></section><section v-if="data.pull" class="panel pull-progress-panel"><div class="panel-head"><div><p class="eyebrow">DOWNLOAD LOG</p><h2>{{ data.pull.name }}</h2></div><span class="state" :class="data.pull.phase">{{ data.pull.phase }}</span></div><pre class="pull-log">{{ data.pull.logs.join('\\n') }}</pre><p v-if="data.pull.phase === 'completed'" class="hint">The verified kernel and rootfs are now available in the local runtime image store.</p></section></template>
