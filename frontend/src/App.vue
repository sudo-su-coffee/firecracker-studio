...
const state = reactive({ tab: 'overview', connectionURL: '', source: 'alpine:latest', sourceType: 'oci', baseProfile: 'alpine', imageName: '', artifactDigest: '', vcpus: 1, memoryMiB: 512, selectedVM: null, serverID: '', showWorkers: false, showAddWorker: false, newServer: { name: 'Remote worker', url: '', kind: 'remote', username: '', token: '' } })
...
<template>
  <div class="desktop-shell">
    <aside class="sidebar">
      ...
      <div class="sidebar-bottom"><button class="ghost small" @click="state.showWorkers = true">Workers</button><small>v1.0.4 · BlackLoverTech</small></div>
    </aside>

    <main class="main-area">
      <header class="topbar"><div><p class="eyebrow">{{ tabs.find(tab => tab.id === state.tab)?.label.toUpperCase() }}</p><h1>{{ state.tab === 'overview' ? 'Workspace' : tabs.find(tab => tab.id === state.tab)?.label }}</h1></div><div class="top-actions"><span class="connection-pill" :class="{ online: data.health }">{{ data.health ? '● Connected' : '○ Offline' }}</span><button class="ghost" @click="state.showWorkers = true">Workers</button></div></header>
    ...
    <div v-if="state.showWorkers" class="modal-backdrop" @click.self="state.showWorkers = false"><section class="modal panel server-manager"><div class="panel-head"><div><p class="eyebrow">CONNECTIONS</p><h2>Workers</h2></div><button class="ghost" @click="state.showWorkers = false">×</button></div><p class="section-note">This server is connected automatically. Add a remote worker only when you need another host.</p><div v-for="server in data.servers" :key="server.id" class="server-row"><div><strong>{{ server.name }}</strong><small>{{ server.kind === 'local' ? 'Local' : 'Remote' }} · {{ server.url }}</small></div><span class="state" :class="server.health">{{ server.managed ? 'connected' : server.health }}</span><button class="ghost small" @click="checkServer(server.id)">Check</button><button class="primary small" :disabled="server.health !== 'healthy'" @click="state.serverID = server.id; switchServer(); state.showWorkers = false">Use</button><button v-if="!server.managed" class="ghost small danger" @click="removeServer(server.id)">Remove</button></div><div class="manager-actions"><button class="primary small" @click="state.showAddWorker = true; state.showWorkers = false">+ Add remote worker</button><button class="ghost small" @click="state.showWorkers = false">Done</button></div></section></div>
